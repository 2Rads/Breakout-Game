...
namespace Breakout_Game
{
    public class Paddle
    {
        ...
        public void move(int direction, PictureBox DisplayBox)
        {
            ...
            {
                paddle = new RectangleF(new PointF(paddle.X + direction * SPEED, paddle.Y), new SizeF(LENGTH, 15));
            }
            //DisplayBox.Right is 11 more that it should be
            if (paddle.Right > DisplayBox.Right - 11)
            {
                paddle = new RectangleF(new PointF(DisplayBox.Right - paddle.Size.Width - 11, paddle.Y), new SizeF(LENGTH, 15));
            }
            if (paddle.Left < 0)
            {
                paddle = new RectangleF(new PointF(0, paddle.Y), new SizeF(LENGTH, 15));
            }
        }
    }
}
